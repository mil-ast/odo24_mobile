...
class PasswordRecoveryCubit extends Cubit<PasswordRecoveryState> {
  ...
  Future<void> submitForm({
    ...
    try {
      ...
      final time = _sharedPreferences.getInt(_spKeyTimeSendCode) ??
          DateTime.now()
              .subtract(Configs.sendEmailDuration + const Duration(seconds: 1))
      ...
      if (dt.add(Configs.sendEmailDuration).isBefore(now)) {
        ...
        _sharedPreferences.setInt(
            _spKeyTimeSendCode, now.millisecondsSinceEpoch);
      }

      emit(PasswordRecoveryState.codeSentSuccessfully(
        email: email,
        password: password,
      ));
    } catch (e, st) {
  ...
  Future<void> recovery({
    ...
    try {
      ...
      emit(PasswordRecoveryState.failure(
        AppException(
